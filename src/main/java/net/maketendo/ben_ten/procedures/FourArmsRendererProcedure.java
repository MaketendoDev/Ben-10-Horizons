...
public class FourArmsRendererProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if (((entity.getCapability(Ben10ModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new Ben10ModVariables.PlayerVariables())).OmnitrixAlien).equals("Four Arms")) {
			...
			if (_evt.getRenderer() instanceof PlayerRenderer && !(_evt.getRenderer() instanceof com.kleiders.kleidersplayerrenderer.KleidersIgnoreCancel)) {
				...
				if (ResourceLocation.tryParse("ben_10_horizons:textures/entities/Four_arms.png") != null) {
					_texture = new ResourceLocation("ben_10_horizons:textures/entities/Four_arms.png");
				}
				Modelfour_arms newModel = new Modelfour_arms(context.bakeLayer(Modelfour_arms.LAYER_LOCATION));
